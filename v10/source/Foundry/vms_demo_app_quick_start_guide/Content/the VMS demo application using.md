...
11.  **FCM Project URL and Service Account Key**: Enter the **FCM Project URL** and upload **Service Account Key** to install and run the Engagement demo application on Android devices. For more details about how to create the FCM Project URL and Service Account Key, refer to, [Engagement Guide to build sample app for Android](../../../Foundry/vmf_msg_quick_start_guide_build_sample_app_android/Content/Introduction.md).
    
     When the FCM Project URL and Service Account Key are created, a project ID also is created for the project name. The project ID, FCM Project URL and Service Account Key are used to configure Engagement server and to run the client application.
    
     ![](Resources/Images/image008.png)
    
12.  If the entered **FCM Project URL** and **Service Account Key** are correct, the system displays the confirmation message that the cloud connection is successful. Click **Ok** to continue.
