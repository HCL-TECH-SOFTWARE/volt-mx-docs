...
Before using Volt MX SDK APIs for Android, you must configure Volt MX Android SDK (kony_sdk-release.aar) into your IDE.

To configure kony_sdk-release.aar file, follow these steps:
...
7.  To import SDK libraries to your project, follow these steps:
    1.  Unzip the `HCLVoltFoundry-android-sdk.zip`.
    2.  Copy the `kony_sdk-release.aar` and paste it to `/app/libs/` folder in your project.
    3.  In app level  gradle file add this line: 
    
        ```
        implementation files('libs\\kony_sdk-release.aar')
        ```
...
10.  Add the following services in your `AndroidManifest.xml`:

      <code class="language-plaintext highlighter-rouge" style="background-color:white;border:none">&lt;service
        android:name="com.kony.sdkcommons.Network.NetworkCore.KNYHttpJobIntentService"<br/>
        android:exported="false"
        android:permission="android.permission.BIND_JOB_SERVICE"&gt;&lt;/service&gt;</code>
    

      <code class="language-plaintext highlighter-rouge" style="background-color:white;border:none">&lt;service
     android:name="com.kony.sdkcommons.Network.NetworkCore.KNYHttpService"
     android:exported="false"&gt;&lt;/service&gt;</code>
