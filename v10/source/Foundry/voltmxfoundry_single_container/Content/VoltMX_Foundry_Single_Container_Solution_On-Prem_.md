...
*   Install `Docker Engine version 28.0.4` and `Docker Compose version 2.34.0`.
    
    > **Note:** For Windows and macOS, you can download and install the package installer for `Docker Desktop version 4.40.0`. For Linux, you must install Docker Engine and Docker Compose separately.  
...
All the Docker images built for Volt Foundry are built using the `Tomcat version 9.0.107-jdk11` as the base image.
...
The docker images will be pulled when you execute the installation scripts in the "HCL Volt Foundry Container Installer" found on HCL's download website. To download "HCL Volt Foundry Single Container Installer (Docker)", [log on to](https://my.hcl-software.com/downloads/voltmx/foundrycurrent).
...
*   **FABRIC_BUILD_VERSION** and **FABRIC_DATABASE_BUILD_VERSION** - For each, the value should be in the format "10.0.0.0_GA". This should correspond to the Foundry Docker image version you are planning to install/upgrade. The Foundry Docker images are available at [https://hclcr.io/harbor/projects/47/repositories](https://hclcr.io/harbor/projects/47/repositories). If you are not sure about the version of Foundry to install or upgrade, please contact HCL support.
...
1.  Download the **` VoltFoundrySingleContainer-<version>_GA.zip`** from [downloads](https://my.hcl-software.com/downloads/voltmx/foundrycurrent) and extract it. The file structure will be as shown below:
...
*   **How should I restart a container that has been killed?**
    ...
    *   Go to Downloads (the folder you downloaded and extracted it to) > `VoltMXFoundrySingleContainer-<version>-GA/foundry-<NUMBER>/` folder. &lt;NUMBER&gt; is a random number generated during installation.
...
*   **How to configure the IP address as the hostname to avoid an unknown host exception?**
    ...
    2.  Update `docker-compose.yml` file under `Downloads/VoltMXFoundrySingleContainer-<version>-GA/foundry/`
