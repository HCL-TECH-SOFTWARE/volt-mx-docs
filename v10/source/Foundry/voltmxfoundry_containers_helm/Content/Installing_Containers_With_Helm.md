...
- Kubernetes Cluster running version above 1.27.6.

    >**Note**: Customers who plan on installing Foundry with SSL will need to import the certificate into cacerts before Foundry is installed.
...
Download the Foundry Helm chart from [MHS portal]( https://my.hcl-software.com/downloads/voltmx/foundrycurrent).
...
1. Download the Helm charts from [MHS portal]( https://my.hcl-software.com/downloads/voltmx/foundrycurrent) and unzip the contents.   With a command prompt, cd to the root of the unzipped content (you will see `values.yaml` here along with Charts in the subdirectories named `apps` and `dbupdate`).<br><br>
    **Note:**
