...
This section explains how to configure Volt MX Foundry components in the JasperReports Server. Based on the configuration of Volt MX Foundry components, <b>such as accounts</b>, server\_URL, the JasperReports Server provides data to Volt MX Foundry Console. The data is displayed in Volt MX Foundry Console's **Reports** page.
...
2.  In the `applicationcontext-security-web-oauth.xml` file, configure the following:
    ...
    *   Configure the value for the `<property name>` to enable   
        the JasperServer Professional Edition or JasperServer Enterprise Edition. For example, if you set the value to true, the JasperServer Professional Edition is configured. If you <B>set the value</B> to false, the JasperServer Enterprise Edition is configured
...
4.  In the `js.config.properties` file, replace  `%%SERVER_URL%` with the JasperServer URL.  
    ...
    > **_Note:_**  Enter credentials for jasperadmin. For Jasper Reports Server  V8.2.0/7.9.2/7, the default credentials for jasper admin are: <br>
