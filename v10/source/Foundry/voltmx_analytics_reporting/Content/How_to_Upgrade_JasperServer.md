...
This section explains how to upgrade Jasper Reports Server from `V 7.9.2` to `V 8.2.0`.
...
2.  Back up your JasperServer Database.
    ...
    2.  ```
           <jrs-server-home>/postgresql/bin/pg_dump.exe --host=<host> --port=<port> --username=<username> jasperserver  >  js-db-7.9.2-dump.sql
        ```
...
3.  Export the current Repository Data, as follows:
    ...
    *   Command for Windows
    ...
    1.  ```
        cd <js-install-7.9.2>/buildomatic
        ```

    2.  ```
        js-export.bat --everything --output-zip js-export-7.9.2.zip
        ```
    ...
    *   Command for Linux
    ...
    2.  ```
        js-export.sh --everything --output-zip js-export-7.9.2.zip
        ```

4.  Download the JasperReports Server 8.2.0 WAR File Distribution. The WAR file distribution comes in a compressed ZIP file named TIB_js-jrs_8.2.0_bin.zip.Download the WAR file distribution from TIBCO Jaspersoft edelivery website.


5.  Extract all files from the TIB_js-jrs_8.2.0_bin.zip to a local system - for example, &lt;js-install-7.9.2&gt;(C:\Jaspersoft on Windows, /home/&lt;user&gt; on Linux).
...
6.  Configure the buildomatic for your Database and Application Server, as follows:
    ...
    a.  Copy the `postgresql_master.properties` configuration file located in `<C:\Jaspersoft\TIB_js-jrs_8.2.0_bin\jasperreports-server-pro-8.2.0-bin\ buildomatic/sample_conf/postgresql_master.properties>`  

    b.  Paste the `postgresql_master.properties` file to `<C:\Jaspersoft\TIB_js-jrs_8.2.0_bin\jasperreports-server-pro-8.2.0-bin\ >/buildomatic.` 
...
7.  Upgrading to JasperReports Server 8.2.0
    ...
    ```
     ...
     b.	Start your Database Server(PostgreSQL Server).
        ...
        cd &lt;TIB_js-jrs_8.2.0_bin&gt;/buildomatic
        Please use the js-7.9.0-export.zip ‘file path location’ from above step here: <br>
        <b>Windows</b>:<br>
        js-upgrade-newdb.bat <path>\js-7.9.2-export.zip <br>
        <b>Ubuntu Linux</b>:<br>
        ./js-upgrade-newdb.sh <path>/js-7.9.2-export.zip  <br>


     Output Log Location: The output logs are available at <js-install-7.9.2>/buildomaticlogsjs-upgrade-<date>-<number>.log

8. Download hotfix_JRSPro8.2.0_cumulative_20240822_0330 from TIBCO Support

9. Apply the hotfix_JRSPro8.2.0_cumulative_20240822_0330 hotfix. Follow readme.txt file in the hotfix_JRSPro8.2.0_cumulative_20240822_0330\jasperserver-pro and
hotfix_JRSPro8.2.0_cumulative_20240822_0330\js-install to apply the hotfix


10.	Start and Log into JasperReports Server 8.2.0.

    a.	Start your application server (Tomcat server).

    b.	Clear your browsing cache.  

    Log in to the JasperReports Server. You can view the JasperReports Server version upgraded to V8.2.0. <br>
...
13.	Clear the Repository Cache Database table.
    ...
    Now your JasperReports Server is upgraded to V 8.2.0, and all the analytics information from the previous version is available in the upgraded version.  
