...
> **_Note:_** In the latest version of Flyway (the tool used to create and update product databases), any value following a dollar sign (`$`) is interpreted as a variable or identifier. To prevent unexpected behavior, do not include the `$` character in the database password. 
