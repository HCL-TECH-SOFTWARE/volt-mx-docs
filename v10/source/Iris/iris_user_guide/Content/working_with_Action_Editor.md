...
*   The Properties Editor always displays the properties of the most recently selected widget. If you switch to an action sequence or code, regardless of what widget it is associated with, the Properties Editor will continue to display the properties of the most recently selected widget, and those properties will not be currently editable.
    ...
    > **_Note:_** If your project was originally created using Volt Iris 2.5 that was then imported into Volt MX Studio 6.5, and you import it into the latest version of Volt Iris, the action sequences are not imported.
    

Add Watch Actions
-----------------

In Volt Iris, you add action sequences for Apple Watch as functions written in the Swift programming language; watchOS does not support JavaScript. For more information, see [Swift](https://developer.apple.com/swift/) on the Apple Developer site.
...
2.  In Design View and Diagram View, Volt Iris points out statements in the action sequence that are in error or are incomplete with a red information icon. Correct these statements.
...
4.  Volt Iris evaluates the action sequence. If the code is not valid. Volt Iris displays an error message in a banner as shown in Fig. 1.0.


      
    ![](Resources/Images/Error_banner.png)
...
5.  After you correctly revise the sequence, Volt Iris verifies the validity of the action sequence and displays an appropriate success message.
...
2.  Delete using icon on the action node
