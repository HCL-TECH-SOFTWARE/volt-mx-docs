...
> **_Note:_** Form Iris 9.5.47 onwards Iris only accepts JDK 17 from the build preferences.  But for Cordova application below are the changes<br>i) In order to build the cordova applications JDK 11 must be used as Cardova does not support JDK 17 yet.<br>ii) Path of the JDK 11  and Gradle home must be provided in the androidbuild.properties file to ensure successful Cordova build.<br><b>Example:</b>
<br>`cordovaJavaHome = /Library/Java/JavaVirtualMachines/jdk-11.jdk/Contents/Home
gradleHome=/Library/Documents/gradle`
