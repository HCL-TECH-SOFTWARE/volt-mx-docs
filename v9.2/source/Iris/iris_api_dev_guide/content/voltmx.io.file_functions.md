...
<blockquote>
    <em><b>Note: </b></em>
    <ul>      
        If target version is 33 and above below media permission is required to add in IRIS project setting -> Native-> Android Mobile/Tablet -> Tags (Child tag entries under < manifest > tag) depending on the target version.<li>< uses-permission android:name="android.permission.READ_MEDIA_AUDIO"/>.
    </ul>
</blockquote>
