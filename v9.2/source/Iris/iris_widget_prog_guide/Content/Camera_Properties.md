...
<details close markdown="block"><summary>cameraPreviewMode Property</summary>

* * *

Specifies how the camera preview will be displayed on the device.
...
*   PREVIEW\_FIT\_TO\_FULL\_SCREEN (Default Value)

> **_Note:_** The property is only supported when the property _enableOverlay_ is set to true.

The following are the available options:

- PREVIEW\_FIT\_TO\_FULL\_SCREEN – Allows the preview to cover the full
screen.
- PREVIEW\_FIT\_AS\_PER\_ASPECT\_RATIO – Allows the preview to maintain the
best possible aspect ratio for the current screen dimensions which are supported from
the android platform on that device. This may result in visible bands or empty spaces.

### Example

```
this.view.camerawidget.cameraPreviewMode=constants.PREVIEW_FIT_AS_PER_ASPECT_RATIO;
```
