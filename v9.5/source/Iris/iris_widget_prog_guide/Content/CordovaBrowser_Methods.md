...
CordovaBrowser Methods
======================

The CordovaBrowser widget is associated with the following methods:
...
This method specifies whether the CordovaBrowser widget can navigate back into history.
...
This method specifies whether the CordovaBrowser can navigate forward into history.
...
If the browser cache is empty then this method returns _false_ and the [goForward](#goForwar) method has no effect.
...
This method clears the page history of the specified CordovaBrowser.
...
| Scenario | Android | iOS |SPA |
| --- | --- | --- | --- |
| If there is any error in evaluating the JavaScript | Null | Null |VoltMXError |
| If return is used (outside any defined function) | Null | Null | VoltMXError |
| If return type is string | Works fine | Works fine | Null |
| If return type is non-string | Works fine | Works fine | Null |
...
When there is an error in the JavaScript evaluation, the exception type VoltMXError displays error code 106, Unknown Error.

Cross-domain execution is not possible and displays VoltMXError 104, not supported error.
...
// throws error and returns null.
webWidget1.evaluateJavaScript ("return document.title");

//returns the document.title of the web page loaded.
webWidget1.evaluateJavaScript ("document.title");

//If the script loaded in the webWidget1 is a var a = "100"; then calling-
...
This method returns null if there are any JavaScript evaluation errors. It throws exceptions if the JavaScript string specified is more than 10MB or If the JavaScript is taking more than 10 seconds to execute.
...
For more information on security risk associated in using the `addJavascriptInterface` method, refer to [http://developer.Android.com/reference/Android/webkit/WebView.html#addJavascriptinterface(java.land.Object,java.land.String)](http://developer.android.com/reference/android/webkit/WebView.html#addJavascriptinterface(java.land.Object,java.land.String)).

To explicitly enable the `evaluateJavaScript` method, set the `enableJsInterface` writable JS property on the CordovaBrowser Widget to _true_.
...
This method returns null if there are any JavaScript evaluation errors and run-time exceptions. To handle run-time exceptions you must keep the script in try catch block and return the error string from catch so that this method returns the error string.

For example:

```

var result = webWidget1.evaluateJavaScript(" (function() { try {// abcd } catch (err) { return ('ERROR');})");
...
//Example2 
var result = webWidget1.evaluateJavaScript("function f() {var a = 5;a += 10;};");
voltmx.print(" >> Result: "+result + " << ");
//output of example 2
//>>Result:null<<
...
*   Available on iOS, Android and SPA platforms
...
```
    ...
    webWidget1.evaluateJavaScriptAsync ("return a"); //returns null.
```

Android

*   There is a security risk for using this method in versions earlier than 4.2 devices for all targets, and 4.2 and 4.3 devices with target less than 17. This method is disabled in those versions. For more information, refer to [http://developer.Android.com/reference/Android/webkit/WebView.html#addJavascriptinterface(java.land.Object,java.land.String)](http://developer.android.com/reference/android/webkit/WebView.html#addJavascriptinterface(java.land.Object,java.land.String)). To enable this method, use the "enableJsInterface"property for the CordovaBrowser widget. If you are creating the widget through IDE, you should set this property in preShow of the Form.
```
...
```
    ...
    var result = webWidget1.evaluateJavaScriptAsync("(function() {try {//abcd} catch (err) {return 'ERROR';}})", callbackJavascriptAsync);
    
    function callbackJavascriptAsync(result, error) {
     if (result == "ERROR") {
      voltmx.print(">>Unown Error<<");
     } else {
      voltmx.print(">>Result:" + result + "<<");
     }
    }
...
//Example1 
webWidget1.evaluateJavaScriptAsync("eval(2+3)", function(result, error) {
 voltmx.print("Result:" + result);
});
...
//Example2 
var result = webWidget1.evaluateJavaScriptAsync("function f() {var a = 5;a += 10;};", function(result) {
 voltmx.print("Result: " + result);
});
...
//Example3 
try {
 var result = webWidget1.evaluateJavaScriptAsync("function f() {var a = 5;a += 10;}", function(result, error) {
  if (!error) {
   voltmx.print(" Result: " + result);
  } else {
   voltmx.print("Error code: " + error.errorCode + "Message: " + error.message);
  }
 });
...
//output error code:106 Message: Unknown Error
```
...
Returns a list of files that match the resource type specified in the _RESTYPE_ parameter.FFFF
...
Available on all platforms except SPA, and Server side Mobile Web
...
Retrieves a Boolean value that indicates whether your app can open an HTMP preview.
...
//Sample code to invoke isHtmlPreviewEnabled method for a CordovaBrowser widget.
var status= frmCBrowser.myCBrowser.isHtmlPreviewEnabled();
...
//Sample code to invoke isWebAppDevelopmentEnabled method for a CordovaBrowser widget.
var status= frmCBrowser.myCBrowser.isWebAppDevelopmentEnabled();
alert("Is Cordova Apps Enabled in the Browser widget?::" + status);
```
...
This method enables you to load the data into the CordovaBrowser widget with the provided configuration.
...
The contents of the resource file or image that has to be loaded into the CordovaBrowser. The contents should be raw bytes returned by the camera or voltmx.io.File API.
...
1.  This method should be called on the CordovaBrowser widget from the post show of the form.
...
//Sample code to invoke loadData method in a CordovaBrowser widget.
...
// Sample code to invoke reload method by using a CordovaBrowser widget.
frmCBrowser.myCBrowser.reload();
```
