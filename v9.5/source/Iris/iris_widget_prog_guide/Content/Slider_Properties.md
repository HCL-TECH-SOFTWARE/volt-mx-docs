...
When thumbImage and thumbTintColor are both configured, thumbTintColor takes precedence over thumbImage.

From iOS 26 onward, both thumbImage and thumbTintColor are applied.

To preserve the liquid‑glass effect, do not provide thumbImage. Any change to slider properties (colors or images) disables the liquid‑glass effect entirely.
...
*   SLIDER\_VIEW\_TYPE\_DEFAULT: Specifies the default slider view. An indicator is displayed on the slider widget. 
*   SLIDER\_VIEW\_TYPE\_PROGRESS:Specifies the progress view of the slider. The indicator is not displayed. It represents the progress of an activity that is being completed in percentage. For Desktop Web platform, the property selectedValue is not available to set through IDE, but it can be set through code. 
