...
The following methods are associated with the TextBox Widget:
...
<details close markdown="block"><summary>getSelection Method</summary>

* * *

This method returns a JavaScript object that contains the keys {startIndex, endIndex} of a TextBox Widget.
...
Index of the beginning of the selected text.

endIndex

Index of the end of the selected text.

> **_Note:_** If the text is not selected in the TextBox, both the keys contain the index of the character that follows the input cursor.

This method returns null if the TextBox is not rendered in the UI when this method is invoked. This method returns start index and end index {startIndex:0, endIndex:0} when this method is invoked after the form is displayed and the focus is not on the TextBox. This method returns the cursor position when the focus is on the TextBox.

Example

```
//Sample code to invoke the getSelection method of a TextBox widget.

var selectedIndex=frmTxtBox.myTxtBox.getSelection();
```
...
<details close markdown="block"><summary>setSelection Method</summary>

* * *

This method allows you to move the cursor position to the specified start index. It also allows you to set the start and end positions of the current text selection in a TextBox Widget.
...
Specifies the index of the first selected character.

endIndex

Optional. Specifies the index of the character after the last of the selected character.

When the startIndex is set or startIndex and endIndex are same, no text will be selected and the cursor moves to the start position. If either of the parameter is greater than the length of text in the TextBox, it is treated as pointing to the end of the text. If endIndex is less than startIndex, then both are treated as the value of endIndex and the cursor moves to the endIndex position.

Remarks

If the values of startIndex and endIndex are negative, the value is treated as '0'.

If the TextBox is not rendered in the UI when this method is invoked, this method will be ignored.

If the TextBox is not in the visible area when the method is invoked, this method brings the TextBox into the visible area and sets the specified cursor position. Also, opens up the keyboard while it sets the cursor.

The setSelection and getSelection methods are not supported:

*   when the viewType property is set to _TEXTBOX\_VIEW\_TYPE\_SEARCH\_VIEW_ in the Android and iOS platforms.
*   when the TextBox has text more than its width, the last part of the text comes into the visible area of the TextBox when trying to set cursor, in the iOS, SPA, and Desktop Web platforms. But when you start typing, the text is displayed at the correct cursor position.
*   when the TEXTBOX\_INPUT\_MODE\_NUMERIC is set to true in the textInputMode property in the SPA platforms. But works in the SPA Windows and Desktop IE.


In the Android and iOS platforms, the setSelection and getSelection methods are not supported when the viewType property is set to _TEXTBOX\_VIEW\_TYPE\_SEARCH\_VIEW_.

In the iOS, SPA, and Desktop Web platforms, when the TextBox has text more than its width, the last part of the text comes into the visible area of the TextBox when trying to set cursor. But when you start typing, the text is displayed at the correct cursor position.

In the SPA platforms, the setSelection and getSelection methods are not supported when the TEXTBOX\_INPUT\_MODE\_NUMERIC is set to true in the textInputMode property. But works in the SPA Windows and Desktop IE.

Example

```
//Sample code to invoke the setSelection method of a TextBox widget.

frmTxtBox.myTxtBox.setSelection(5);
