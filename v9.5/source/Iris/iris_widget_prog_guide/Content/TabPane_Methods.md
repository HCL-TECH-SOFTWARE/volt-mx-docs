...
This section describes the methods associated with the TabPane widget. You can use these methods across all platforms.
...
<details close markdown="block"><summary>addTab Method</summary>
...
addTab(tabId,tabName,tabImage,box,masterDataLoad);
```
...
Specifies the image of the tab. This can be a string referencing a local resource or an image Object (voltmx.image). You can create an image Object by using voltmx.image Namespace functions.
...
/*Sample code to invoke addTab method by using a TabPane widget where the app.png image is available in the resources folder.*/

frmTab.myTab.addTab("tab1", "LocTAB", "app.png", box1, {});
```

Using an image object for tabImage.
...
//addTab method call 
tabPane.addTab("tPane", "LocTAB", imgObjRef, box1, {
    Your masterDataLoad Here
});
```
...
<details close markdown="block"><summary>addTabAt Method</summary>

* * *

This method is used to add a tab at the specified index to the TabPane. The new tab is placed before the specified index.

### Syntax
```

addTabAt(tabId, tabName, tabImage, box, index);
```
...
If the row Index mentioned is greater than "N", then row is added at the end of the tabpane ui and if it is less than zero, it will be added at the beginning of the TabPane.

### Example

```

/*Sample code to invoke addTabAt method by using a TabPane widget where the TabImage.png image is available in the resources folder.*/

frmTab.myTab.addTabAt("tabNew", "TabDynamic", "TabImage.png", tabNew, 0);
```
...
var tabPane = new voltmx.ui.TabPane(tabPaneBasic, tabPaneLayout, tabPanePSP);
var imgObjRef = voltmx.image.createImage("local.png");
var tabNew = new voltmx.ui.Box(tabBasicConfig, tabLayoutConfig, tabPspConfig);

//addTabAt method call    
tabPane.addTabAt("tabNew", "TabDynamic", imgObjRef, tabNew, 0);
```
...
<details close markdown="block"><summary>removeTabAt Method</summary>
...
Index represents the tabindex in the order in which the tab originally was added. Index is zero based in JavaScript. If the index is not within the limits then _removeTabAt_ will be silent and doesn't yield any results.

An active tab (currently selected tab) can be removed. If an active Tab is removed, then the first index in activeTabs property will be selected/focused. If active Tabs has only one element, or view doesn't have more than one active tab, then if the active tab is closed, the 0th position will be selected by default.

### Example

```

//Sample code to invoke removeTabAt method by using a TabPane widget.

frmTab.myTab.removeTabAt(2);
```
...
<details close markdown="block"><summary>removeTabByld Method</summary>
...
//Sample code to invoke removeTabById method by using a TabPane widget.  
 //Here, tab1 is a tab inside myTab TabPane widget.

frmTab.myTab.removeTabById("tab1");
```
