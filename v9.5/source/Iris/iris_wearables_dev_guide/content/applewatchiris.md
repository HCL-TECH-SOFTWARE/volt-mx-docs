...
1.	Right click the **Notifications** node in the Volt Iris Project Explorer under Wearables/Apple Watch.  
2.	From the menu, select **New Notification**. By default, this creates a **Static Notification**. It displays only information and alerts.
3.  Add label to the **Static Notification** form. You must not add any other widgets, except label.
    
    > **_Note_**: You can add only one label to the **Static Notification**.

4.  Select **Static Notification** which you have created in step 2. Then select **Static Notification** properties pane as displayed in below image. And then select the created label from the **Notification Label** property.
    
    ![](resources/images/apple_watch_iris33.png)

5.	If you want to change the notification to a dynamic notification, which can have interactive elements such as buttons, select the notification that you created in step 2.  
6.	Use the down arrow next to the name of the notification to pull down the context menu and click **Create Dynamic Notification**, as shown in the following illustration.  
