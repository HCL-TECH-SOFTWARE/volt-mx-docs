...
> **_Note:_** From V9 SP2 onwards, the i18n database data for a Volt MX App child app is stored in child app data and not under the parent app. This feature is applicable for iOS and Android platforms.
...
inputAccessoryViewType i18n Keys:
----------------------

The [`inputAccessoryViewType`](https://help.hcl-software.com/voltmx/v9.5/Iris/iris_widget_prog_guide/Content/FlexForm_Properties.html#syntax_33) property supports internationalization (i18n) on supported widgets. When a widget uses `inputAccessoryViewType` to display custom accessory views above the keyboard, specific keys are used to localize text within these views. The following table lists the supported keys for i18n.

| Key | Description |
| --- | --- |
| platform.nextprevtoolbar.done | Key for the "Done" button. |
| platform.canceltoolbar.cancel | Key for the "Cancel" button. |

* Supported keys must be added under <b>Project Settings > Internationalization (i18n) > Configure</b>.
* `inputAccessoryViewType` is only supported in iOS.
