...
> **_Note:_** From V9 SP2 onwards, the i18n database data for a VoltMX App child app is stored in child app data and not under the parent app. This feature is applicable for iOS and Android platforms.
...
> **_Note:_** On iPhone, Android, and DesktopNative, the device settings mandate that a region must also be associated with a language.
...
| Return Value | Description |
| --- | --- |
| listOfLocales \[Table\] | A table with the following key-value pairs is returned:languagecountryname For example, `local list = voltmx.i18n.getSupportedLocales()` |
...
*  if you use the **isI18nLayoutConfigEnabled** key and the **voltmx.application.setApplicationLayout** API together in a single application, the application does not function as expected.
