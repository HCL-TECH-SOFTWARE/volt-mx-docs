...
<tr>
<td></td>
<td>Enable SceneDelegate</td>
<td>Apple recommends that all applications migrate from the traditional AppDelegate approach to the modern SceneDelegate architecture. As per <a href="https://developer.apple.com/documentation/technotes/tn3187-migrating-to-the-uikit-scene-based-life-cycle">Apple's technical note</a>, future iOS releases will no longer support launching applications that rely solely on AppDelegate.
To align with this direction, VoltMX IRIS has added support for SceneDelegate starting from <b>version 9.5.</b> This enhancement enables developers to adopt the scene-based app structure recommended by Apple. To enable SceneDelegate from IRIS developers need to turn on the Enable SceneDelegate flag which is available at <b>project settings-> Native -> iPhone/iPad->iOS</b> Build Settings.<br>
<b>Note:</b> If this flag is enabled for an application, then the extending UIApplicationDelegate won’t work. This is because the CarApp relies on the SceneDelegate lifecycle to manage the application’s state and events. To extend the lifecycle events of UISceneDelegate, developers must update their implementation to extend UISceneDelegate and override the appropriate delegate methods.<br>
For more details, please refer to the <a href="https://help.hcl-software.com/voltmx/v9.5/Iris/iris_widget_prog_guide/Content/PlatformSpecificCapabilities.html">Extending SceneDelegate</a> section.</td>
</tr>
<tr>
<td></td>
...
## Icon Settings in iPhone/ iPad
With the releases of iOS 18 & Xcode 16.2, Apple provided a new set of app icons and also introduced more flexibility for configuring app icons to support different themes, namely default, dark, and tinted. For every app icon size, there is an option to configure the app icon for default, dark & tinted mode.
...
### Configuring App Icons in Iris  

Open Volt Iris Project Settings and navigate to category Native -> iPhone/iPad. App icons to configure are organized by device and theme. Icons for each theme are housed in respective sections that can be expanded/collapsed. 


### Add a new app icon

Let us take an example of configuring the icon for Notification-iOS 7-11 20pt 2x. 
Following are the steps.  

1. Hover on the square with a dotted border. This will show an add icon in the center.
2. Click on the add icon. A popup with available images is displayed.
3. Select image, and then click on the OK button to dismiss the popup. Preview of the selected image is displayed in the respective box for the app icon. 


<details close markdown="block"><summary>Click to view the image</summary>

![](Resources/Images/Settings_iPhone_AddIcon.png)
</details>
...
Let us take an example of removing the icon for Notification-iOS 7-11 20pt 2x. 
Following are the steps.
1. To remove an image, hover over the box with the selected image.
2. A delete icon appears at the top right corner of the box. Click on it to delete the app icon.

<details close markdown="block"><summary>Click to view the image</summary>

![](Resources/Images/Settings_iPhone_DeleteIcon.png)
</details>
