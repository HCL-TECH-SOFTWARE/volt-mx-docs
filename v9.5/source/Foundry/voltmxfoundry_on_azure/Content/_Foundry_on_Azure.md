...
| --- | --- | --- |
| **SHARED\_SERVER\_DOMAIN\_NAME**<br>SERVER\_DOMAIN\_NAME| This is the domain name property for Foundry Console component. This is not required if shared cluster is already created in previous installations.| kfazure-console.hcl.net|
...
| **SHARED\_AZURE\_RESOURCE\_GROUP** <br>AZURE\_RESOURCE\_GROUP| Specify the name of the resource group that must be created for the cluster. Enter a name that does not conflict with any of the existing resource groups. This is not required for Non-Production if the shared cluster is already installed for the same subscription.| FoundryXYZ|
...
| **SHARED\_DATABASE\_USER\_NAME**<br>DATABASE\_USER\_NAME| Specify a username for the database that is used by the Foundry components. This is not required in Non-Production installation if a shared cluster is already created in the previous installation.| dbclient|
| **SHARED\_DATABASE\_PASSWORD**<br>DATABASE\_PASSWORD| Specify a password for the database that is used by the Foundry components.The following special character $, \*, &, !, (, ), \\ are not supported in DB password.| Test#/123|
| DATABASE\_PASSWORD\_SECRET\_KEY| Specify the decryption key for the database password. This parameter is required only if you are using an encrypted password.| 12345|
...
| ENVIRONMENT\_DB\_PASSWORD| Specify a password for your environment database. This is required only if the selected database is of the type **mysql** and the installation type is Non-Production.| FabR^c123|
