...
> **_Note:_** Passbook template JSON includes a number of internal arrays at Third Level. The third level column displays the parameter data type as REQUIRED or OPTIONAL while creating a sample request payload.For more information about passbook template datatypes, [see]({{ site.baseurl }}/docs/documentation/Foundry/vms_console_user_guide/Content/PassBook_Template/Passbook_Template.html#dding_a_Passbook_Template)
