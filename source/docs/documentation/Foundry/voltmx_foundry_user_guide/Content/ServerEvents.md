...
User Guide: [Integration](Services.html#integration) \> [Advanced Configurations](Advanced_Configurations.html) > Server Events
...
*   **EventManager**: This class is used to subscribe/unsubscribe to an event of a specified topic. You can access Event Manager using Services Manager API in their custom code.
    ...
    {% highlight voltMx %}request.getServicesManager().getEventManager()
    {% endhighlight %}
    ...
    *   **subscribe**: Subscribes the given event subscriber to all events with the specified topic.
    ...
    {% highlight voltMx %}request.getServicesManager().getEventManager().subscribe(<topic>, <subscriber>);  
    
    {% endhighlight %}
    
    *   **unsubscribe**: Unsubscribes the given event subscriber to all events with the specified topic.
        
        {% highlight voltMx %}request.getServicesManager().getEventManager().unsubscribe(<topic>, <subscriber>);
        {% endhighlight %}
*   **EventNotifier**: This class is used to notify subscribers and check the status of events. You can access Event Notifier using Services Manager API in their custom code.
    ...
    {% highlight voltMx %}request.getServicesManager().getEventNotifier();
    {% endhighlight %}
    ...
    *   **notify (with event data)**: Notify all subscribers of the given event based on the topic.
        ...
        {% highlight voltMx %}request.getServicesManager().getEventNotifier().notify(<event_data>);
        {% endhighlight %}
    *   **notify (with topic and data)**: Creates event data internally and notifies all subscribers of the given event based on the topic.
        ...
        {% highlight voltMx %}request.getServicesManager().getEventNotifier().notify(<topic>, <data>);
        {% endhighlight %}
...
> **_Note:_** Refer to the [Foundry Queue Service java document]({{ site.baseurl }}/java_docs_apis/MiddlewareAPI/index-all.html) for more information on Foundry Queue Service APIs.
...
{% highlight voltMx %}public class CustomEventSubscriber implements EventSubscriber {
...
{% highlight voltMx %}@IntegrationEventSubscriber(topics = {"app/test/events", "app/test/events1"}
