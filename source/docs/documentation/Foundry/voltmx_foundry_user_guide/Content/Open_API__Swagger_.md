...
<details close markdown="block"><summary>Advanced</summary>
    ...
    In the **Total Rate Limit** box, type a required value. This limits the total number of requests that are processed by the service.In the **Rate Limit Per IP** box, type a required value. This limits the number of requests based on the IP Address of the request.
    
    You can also override throttling from the Volt MX Foundry App Services Console. For more information, refer to [Override API Throttling Configuration](API_Throttling_Override.html#override-api-throttling-configuration). |
</details>
...
3.  After you configure all the parameters, you can validate the details by testing the service operation. For more information, refer to [Test a Service Operation](Test_a_Service_Operation.html).
