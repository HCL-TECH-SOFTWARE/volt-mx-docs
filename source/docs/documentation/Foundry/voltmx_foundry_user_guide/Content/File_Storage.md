...
User Guide: [Integration](Services.html#integration) \> [Configure the Integration Service](ConfigureIntegrationService.html) > File Storage Adapter
...
> **_Note:_**  
This document is about the default File Storage adapter for Volt MX Foundry. For information on connecting to a custom S3 bucket, refer to [AWS S3 Adapter](S3_File_Storage.html).

Configure a service for File Storage
------------------------------------

To configure a File Storage adapter in [](Configure the Service.html)[Integration Service Definition](ConfigureIntegrationService.html#IntSD) tab, follow these steps:
