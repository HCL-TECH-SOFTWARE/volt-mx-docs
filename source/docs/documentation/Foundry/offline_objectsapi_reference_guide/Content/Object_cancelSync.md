...
{% highlight voltMx %}<HCLObj>.cancelSync(options, onSuccess, onFailure);
{% endhighlight %}
...
{% highlight voltMx %}HCLObj syncObject = new HCLObj("Managers");
...
HCLObj * syncObject = [
    [HCLObj alloc] initWithName: @"Managers"
    error: & error
];
