...
*   Make sure that you configure a device pool before you test the application  
    For information about device pools, refer to [Configuring Device Pools](Configuring_Device_Pools.html)
...
*   If you are running the test on App Factory, refer to the chrome driver path and browser path as follows:{% highlight voltMx %}System.setProperty("webdriver.chrome.driver", System.getProperty("DRIVER_PATH"));  
...
*   If you are using appscommon.jar in the test code, you can set driver path and browser path as follows:
    ...
    {% highlight voltMx %}System.setProperty("webdriver.chrome.driver", sgConfig.getDriverPath());  
...
3.  To explicitly reference a directory, include the **testresources** tags in the **pom.xml**.{% highlight voltMx %}<testResources>
         <testResource>
...
{% highlight voltMx %}mvn clean package -DskipTests=true  
...
6.  Extract the zip folder. To verify whether the TestNG XML file is present at the desired location (root, by default), extract the jar using **xf** command.
    ...
    {% highlight voltMx %}jar xf nameOfTheProjectFromPom-1.0-SNAPSHOT-tests.jar
    {% endhighlight %}
    
    Alternatively, you can use the **unzip** command to extract the contents of the jar.
    
    {% highlight voltMx %}unzip nameOfTheProjectFromPom-1.0-SNAPSHOT-tests.jar -d sampletestsjarcontents
    {% endhighlight %}
