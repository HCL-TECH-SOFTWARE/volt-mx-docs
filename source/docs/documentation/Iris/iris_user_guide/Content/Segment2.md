...
> **_Note:_** Properties that can be forked are identified by an icon ![](Resources/Images/fork_-un.png) located to the left of the property. For more information, see [Fork a Widget Property](Forking.html#fork-a-widget-property).
...
For more information on enabling the swipe functionality on the rows of a Segment using a row template, refer [SwipeConfig](Flex_Container.html#swipe-config).
...
9.  Map the required [global variables](Global_Variables_and_Data_Store_Keys.html), services, and other items to the Segment **rowData** and **sectionData** as well as to associated rowData and sectionData widgets.  
...
2.  Click **Save**. You have successfully mapped the widgets of the Section Header Template and Row Data of a Segment by using [Mapping Editor](ActionsMapping.html).
