...
[Building Runtime Security in the Application](#building-runtime-security-in-the-application)

[Protecting the Application Binaries](#protecting-the-application-binaries)

[Protection Mechanisms Provided by Volt MX Iris](#protection-mechanisms-provided-by)

[RSA Public/Private Key Pair Generation, Encryption, and Usage](#rsa-key-pair-generation-encryption-and-usage)

[Configuring Project Settings in Volt MX Iris](#configure-project-settings-in)

[Install Finalizer Utility](#install-the-finalizer-utility)

[Impact on App's Performance](#impact-on-app-39-s-performance)

[Application Security Guidelines](#application-security-guidelines)

Building Runtime Security in the Application
--------------------------------------------
...
Protecting the Application Binaries
-----------------------------------

In Volt MX Iris, [_Protected Mode_ option](#configure-project-settings-in) enables several security features that secure the binary at build time by including multiple self-protection security mechanisms. To use the option, enable the **Protected Mode** check box in the **Project Settings** dialog. In iOS, along with selecting the _Protected Mode_ option, you must install a utility tool called **Finalizer**. If an application attack is observed, the security mechanism exits the application.

In this section, you will learn about: 

1.  [Protection Mechanisms Provided by Volt MX Iris](#protection-mechanisms-provided-by)
2.  [RSA Key Pair Generation, Encryption, and Usage](#rsa-key-pair-generation-encryption-and-usage)
3.  [Configure Project Settings](#configure-project-settings-in)
4.  [Install the Finalizer Utility](#install-the-finalizer-utility)
...
3.  Send your public key to [licensing@voltmx.com](mailto:licensing@voltmx.com?subject=Keys).

> **_Important:_** Public key must not be shared with anyone except Volt MX.

5.  VoltMX's security team validates the details and encrypts your public key.
6.  VoltMX's security team then returns the encrypted public key to you through email.
    1.  For iOS, Volt MX provides a set of `fin` keys along with the public key to protect iOS applications.

> **_Important:_** You must not share your private key with anyone including Volt MX. In case of a key compromise, generate a new set of keys and send the public key to [licensing@voltmx.com](mailto:licensing@voltmx.com?subject=Keys)

8.  Navigate to your workspace and create a **\_\_encryptionkeys** folder.
9.  Place the following keys received from Volt MX in **\_\_encryptionkeys** folder.
    1.  Your private key. The private key must be named as `private_key.pem`.
    2.  Encrypted Public key provided by Volt MX Iris (`public_key.dat`).
    3.  For iOS, the `fin` keys provided by Volt MX Iris. These keys are provided for protecting iOS applications.
    
    After you place all your keys, your application folder must look like.
    
    ![](Resources/Images/Fin_Keys_478x236.png)
...
2.  Navigate to **Native >** **iPhone/iPad/Watch** or **Android** tab.
3.  Select **Protected Mode** option.

> **_Note:_**  
\- In iOS, you have to [install the Finalizer utility tool](#install-the-finalizer-utility) as a final step to protect your binaries.  
\- The _Protected Mode_ option works only if the application is built in _Release_ mode.

![](ProtectedModeOption_CRR_527x375.png)

6.  Click **Finish**.
7.  Build the application.

### Install the Finalizer Utility

The final step in protecting iOS binaries is to patch Xcode with a utility tool called Finalizer. The Finalizer utility patches Xcode and provides protection to the application binaries. You can download the utility tool from the Volt MX Developer Portal. Patching Xcode with Finalizer is an important step in protecting your application. The application crashes if the iOS application is built in the _Protected Mode_ and the Xcode is not patched with Finalizer.

> **_Important:_** From Volt MX Iris V8 SP3 GA onwards, you don't have to install Finalizer.

This section covers the following:

[Installation Requirements](#installation-requirements)

[Installing the Finalizer Package](#installing-the-finalizer-package)

[Uninstalling the Finalizer Package](#uninstalling-the-finalizer-package)

#### Installation Requirements

The host Mac on which you run Finalizer must meet the following software and system requirements:

*   **OS**: Mac OS X 10.10 or 10.11
*   **Software**: Xcode 7.1, 7.2, or 7.3.
*   **CPU**: x64-compatible
*   **Compiler**: Apple LLVM compiler 7.0 or 7.2

#### Installing the Finalizer Package

To install the Finalizer package, follow these steps: 

1.  Ensure the host Mac on which you run Finalizer meets the software and system requirements previously listed.
2.  Download Finalizer from the [VoltMX Developer Portal](http://download.voltmx.com/onpremise/finalizer/Finalizer-9.2.0.463f8da-macosx-x64.tar.gz).
3.  Extract the contents of the Finalizer package to a directory of your choice on the host.
4.  Set the necessary environment variables. To do so, source the setenv.sh script as follows:

source <finalizer\_kit\_path>/setenv.sh
...
<finalizer\_kit\_path>/Xcode/deprecated/install-xcode-finalizer-wrapper.sh

10.  Launch Xcode.
11.  Select **KProtected > Run**.
12.  In the **Build Configuration** list, select **Protected**.
13.  Select **Archive** and in the **Build Configuration** list, select **Protected**.

> **_Important:_** If you select the _Protected Mode_ option and do not patch the Finalizer utility with Xcode, your app will crash.

#### Uninstalling the Finalizer Package

To uninstall the Finalizer Package, follow this step: 

1.  Run the `install-xcode-finalizer-wrapper.sh` script with the `-uninstall` argument.

<path\_to\_finalizer\_pkg>/install-xcode-finalizer-wrapper.sh

\-uninstall <path\_to\_Xcode\_installation>
...
![](Resources/Images/AppPerformance_605x293.png)
...
> **_Note:_** For a more comprehensive list, Volt MX Iris recommends the secure coding checklists and best practices maintained by the Open Web Application Security Project (OWASP). [https://www.owasp.org/index.php/OWASP\_Cheat\_Sheet\_Series](https://www.owasp.org/index.php/OWASP_Cheat_Sheet_Series)
...
1.  In Volt MX Iris, on the **Edit** menu (the **Project** menu for Volt MX Iris), click **Internationalization (i18n)**. The Configure Internationalization dialog box displays.
2.  In the Configure Internationalization dialog box, an initial row for an i18n key and the locales you want your app to support is displayed. In the Key field, type the name for your jailbroken key, **ROOT\_DETECTION\_MESSAGE**.
3.  In each locale's field for the ROOT\_DETECTION\_MESSAGE key, enter the word or phrase. Here, for English locale, define your message.
4.  Click **Finish**.
