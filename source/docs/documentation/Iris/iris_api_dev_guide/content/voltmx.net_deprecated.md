...
[![Closed](../Skins/Default/Stylesheets/Images/transparent.gif)](javascript:void(0);)[voltmx.net.invokeServiceAsync](javascript:void(0);)

This API allows you to invoke a service asynchronously with the specified parameters.

> **_Note:_** These APIs are generated by Volt MX Irisfor the code generated using mapping editor. Even though these APIs have been retained as is they have not been published as standard APIs provided by the Volt MX runtime libraries. These APIs will not be available to a developer not using Volt MX Irisin future for development.
...
{% highlight voltMx %}function asyncCallback(status, news) 
			{
			// populate UI
			};
...
>     For example:
>     
>     {% highlight voltMx %}httpheaders={authkey:"myauthkey",  
...
>     
>     {% highlight voltMx %}Authorization:"Basic" <base64encoding of the username:password>". For example, httpheaders={Authorization:  
...
> {% highlight voltMx %}local myhttpheaders={authkey:  
...
>     For example:
>     
>     {% highlight voltMx %}httpconfig={timeout:5}
>     {% endhighlight %}
>     
...
>     For example: 
>     
>     {% highlight voltMx %}httpconfig = { timeout: 5, method: "get"}
>     {% endhighlight %}
...
{% highlight voltMx %}function callbackfunction(status, resulttable)
			{
			if(status == 400)
			{
...
[![Closed](../Skins/Default/Stylesheets/Images/transparent.gif)](javascript:void(0);)[voltmx.net.invokeService](javascript:void(0);)
...
{% highlight voltMx %}local myhttpheaders={authkey="myauthkey",authtoken="myauthtoken"}
