...
| content | Required | Specifies the content of the message and has the following sub-elements: - **mimeType (Optional)**: Specifies the mime type of the content. Default: text/plain - **data (Optional)**: You can use this element to send simple character data (for example, simple text). You must make sure that you specify either the data or the URL element. - **url (Optional)**: You can use this element if you need to access content from a URL. You must make sure that you specify either the data or the URL element. |
