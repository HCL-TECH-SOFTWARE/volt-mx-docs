...
VoltMX Foundry Command Line Installer
=====================================

The VoltMX Foundry Command Line Installer (CLI) allows you to set up a custom VoltMX Foundry deployment in your enterprise. You can install VoltMX Foundry on your Windows and Linux machines using the Command Line Installer.

The VoltMX Foundry CLI aids and supplements the manual install process by providing easy-to-use command line tasks. You can use the VoltMX Foundry CLI if your organization’s policies, standards and software setup requirements need you to perform a custom manual deployment.

Prerequisites
-------------

Following are the prerequisites to install and configure VoltMX Foundry Command Line Installer on your machine:
...
*   Download the Manual Install Artifacts from [VoltMX Downloads](https://community.voltmx.com/downloads/manual).
...
To find out which versions of Application Servers and Databases are supported by VoltMX Foundry CLI, refer to [Supported Application Servers and Databases Guide](https://{../../../Foundry/voltmxfoundry_supported_devices_os_browsers/Content/Introduction.md).

Using the VoltMX Foundry Command Line Installer
-----------------------------------------------
...
```
    ...
    *   **FULLY\_QUALIFIED\_PUBLIC\_URL**: Refers to the URL by which VoltMX Foundry will be accessible post installation. This is an optional field. In case the URL is not provided, it will be derived from the HOST\_NAME, HTTP\_PORT/HTTPS\_PORT properties. This property must be specified in case your VoltMX Foundry environment setup consists of a load balancer/proxy.
```
...
4.  **Properties related to the VoltMX Foundry components**: Following inputs are required for any action which is related to a Foundry component. For example, the generation of WARs or the execution of database scripts.
    1.  **VoltMX Foundry Component selection properties**: The following properties must be set to either true or false. If ALL\_COMPONENTS\_ENABLED is set to true, the rest of the inputs can be left empty. If ALL\_COMPONENTS\_ENABLED is set to false then at least one of the following input properties must be set to true.
        ...
        *   CONSOLE\_ENABLED
        *   APIPORTAL\_ENABLED
    2.  **Properties specific to a VoltMX Foundry component**:
    ...
    3.  **Database Properties**:
        ...
        *   **DB\_USER**: The database user.
            ...
            In case of Oracle, you must provide a user which has the permission for creating users for each of the selected VoltMX Foundry components. The password for all child users would be same as the parent user password.
...
The VoltMX Foundry Command Line Installer supports the following actions:
...
1.  Specify the following properties to generate the artifacts:
    ...
    *   [Properties related to the VoltMX Foundry components](#prop_foundry)
