...
> **Note:**  <br>This API is supported from V8 SP4 onwards.<br>Supported for Windows from V8 SP4 Fix Pack 6 onwards.<br>The column names and values provided as key value pairs are case sensitive.
...
>**Note** : `Cascade Delete` is enabled if the option is checked in `VoltMX Foundry Console >> Object Services >>` under configuring relationships in a data model. When this option is enabled, the child records are deleted recursively along with the parent record. This option is applicable for all platforms except for SPA/Desktop Web channels of Volt MX Iris.
<br>Supported in iOS and Android channels from V8 SP2 onwards.
<br>Supported in SPA/ Desktop Web channels from V8 SP4 onwards.
