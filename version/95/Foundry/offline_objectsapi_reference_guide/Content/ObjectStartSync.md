...
| uploadRequestQueryParams | JSON   | The uploadRequestQueryParams option is used to pass the query parameters in upload network requests.<br><b>Note: </b><br>i) The parameter is supported for Windows, Mobile Web, and Desktop Web channels from release version V8 SP4 FP2.<br>ii) The keys and values in the JSON must be of the string data type.<br>iii) This parameter is not supported for Desktop Native channel. | No  |
